 ...
 * @author USUARIO
 */
@WebServlet(name = "EditDealer", urlPatterns = {"/EditDealer"})
public class EditDealer extends HttpServlet {
            ...
            switch (url) {
                case "edit":
                    ...
                    if (dealer == null) {
                        ...
                        request.getRequestDispatcher("GetDealers?url=getAllDealers").forward(request, response);
                    }
                ...
                case "save":
                    ...
                    dealer = new Dealer(id, firstName, lastName, workedHoursPrice);
                    save(dealer);
                    
                    request.setAttribute("message", "Dealer edited succesful.");
                    request.setAttribute("dealer", dealer);
                    request.getRequestDispatcher("editDealer.jsp").forward(request, response);
    ...
    private void save (Dealer dealer) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("CasinoAppWebPU");
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
