 ...
 * @author USUARIO
 */
@WebServlet(name = "GetMachineType", urlPatterns = {"/GetMachineType"})
public class GetMachineType extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            switch (url) {
                case "consultType":
                    type = getType(id);
                    if (type == null) {
                        request.setAttribute("message", "Machine type not found. It may not exist.");
                        request.getRequestDispatcher("getAllMachineTypes.jsp").forward(request, response);
                    }
                    ArrayList<MachineType> types = new ArrayList<>();
                    types.add(type);
                    request.setAttribute("machineTypeList", types);
                    request.getRequestDispatcher("getAllMachineTypes.jsp").forward(request, response);
                    break;
                case "editType":
                    type = getType(id);
                    ...
                    break;
                default:
                    response.sendRedirect("machineManager.jsp");
            }
    ...
    private MachineType getType (int id) {
